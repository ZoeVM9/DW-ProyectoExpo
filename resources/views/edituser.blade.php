                ...
                <div class="col-auto text-center">
                    ...
                    <div class="card shadow mb-4">
                        <div class="card-header py-3">
                            <h6 class="m-0 font-weight-bold text-primary">Modificar datos
                            </h6>
                        </div>
                        <div class="card-body">
                            <form class="row g-3 needs-validation" action="{{ route('user.update', $user) }}"  method="POST" enctype="multipart/form-data">
                                @csrf
                                @method('PUT')
                                <div class="mb-3">
                                  <input class="form-control" name="id" type="text" placeholder="ID" value="{{$user -> id}}" readonly>
                                </div>
                                <div class="mb-3">
                                  <input class="form-control" name="name" type="text" placeholder="Nombre" value="{{$user -> name}}" required>
                                </div>
                                <div class="mb-3">
                                  <input class="form-control" name="lastname" type="text" placeholder="Apellido" value="{{$user -> lastname}}" required>
                                </div>
                                <div class="mb-3">
                                  <input class="form-control" name="email"  type="email" placeholder="Correo electrónico" value="{{$user -> email}}" required>
                                </div>
                                <div class="mb-3">
                                  <input class="form-control" name="username" type="text" placeholder="Nombre de usuario" value="{{$user -> username}}" required>
                                </div>
                                <div class="mb-3">
                                <div class="mb-3">
                                  <input class="form-control" name="password" type="password" placeholder="Contraseña" value="{{$user -> password}}" required>
                                </div>
                                <div class="mb-3">
                                  <input class="form-control" name="birthdate" type="date" placeholder="Fecha de nacimiento" value="{{$user -> birthdate}}" required>
                                </div>
                                <div class="mb-3">
                                  <input class="form-control" name="phone" type="text" placeholder="Teléfono" value="{{$user -> phone}}" required>
                                </div>
                                

                                <div class="col-md-12  mt-5 text-center">
                                    <button class="btn btn-primary" name="btnnombre" type="submit">Actualizar</button>
                                </div>
                      </form>
                        </div>
                    </div>
